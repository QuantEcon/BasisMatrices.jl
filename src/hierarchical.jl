...
mutable struct HierarcParams{T<:Number} <: BasisParams
    ...
    function HierarcParams{T}(a::T, b::T, r::Int) where T
        r <= 1 && error("refinement level must be >= 1")
        a >= b && error("left endpoint (a) must be less than right end point (b)")
        new{T}( a, b, r)
    end
end

HierarcParams(a::T, b::T, r::Int) where {T<:Number}  = HierarcParams{T}( a, b, r)
HierarcParams(a::T, b::T, r::Int) where {T<:Integer} = HierarcParams(Float64(a), Float64(b),r)

## BasisParams interface
# define these methods on the type, the instance version is defined over
# BasisParams
family(::Type{T}) where {T<:ChebParams} = Hierarc
family_name(::Type{T}) where {T<:ChebParams} = "Hierarc"
...
"""
    nodes(p::HierarcParams)

Get legal nodes ``z\in [0,1]`` from supplied ``x\in [a,b]``
"""
function nodes(p::HierarcParams)
    h = 1.0 / (2.0^p.r)
    Float64[i * h for i in 0:2^p.r]
end
